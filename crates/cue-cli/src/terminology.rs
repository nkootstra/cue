...
pub(crate) fn find_candidates(
    ...
    collect_terms(media_root, media_root, &mut terms, false)?;
    if let Some(root) = context_root {
        let root = fs::canonicalize(root).unwrap_or_else(|_| root.to_owned());
        collect_terms(&root, &root, &mut terms, true)?;
    }
    collect_sibling_transcripts(media_root, output_dir, &mut terms)?;
...
fn collect_terms(
    ...
    for (line_index, token) in tokens(&content) {
        if !token
            .chars()
