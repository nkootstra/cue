...
# by shadowing uname on PATH (via a real dir; /tmp is unreliable here).
set -e
cd /Users/nielskootstra/Code/projects/cue
SHIM_DIR="$PWD/target/shim"
mkdir -p "$SHIM_DIR"
...
EOF
  ...
  local result
  result=$(PATH="$SHIM_DIR:$PATH" ./install.sh --dry-run)
...
chmod +x "$SHIM_DIR/uname"
if PATH="$SHIM_DIR:$PATH" ./install.sh --dry-run > /dev/null 2>&1; then
...
chmod +x "$SHIM_DIR/uname"
PATH="$SHIM_DIR:$PATH" ./install.sh --version 0.2.0 --dry-run | \
